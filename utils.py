...
def build_nano_highlight_prompt(color_name, hex_color, density="thin"):
    ...
    return (
        ...
        f"must become {color_name} (hex {hex_color}). "
        f"Render the colored strands with NATURAL HAIR TEXTURE — individual hair fibers visible, "
        f"subtle highlights and shadows along each strand as real hair catches light, slight variation "
        f"in tone (lighter where light hits, slightly deeper in shadow) — NOT a flat solid block of color. "
        f"The strands must look like real dyed hair extensions, not a painted overlay. "
        f"Match the hue of {hex_color} but allow natural light variation — do NOT make the strands "
        f"look like a flat graphic, a sticker, or a neon paint smear. "
        f"{density_rule}"
