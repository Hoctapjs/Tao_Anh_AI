...
def build_nano_highlight_prompt(color_name, hex_color, density="thin"):
    ...
    return (
        ...
        f"CRUCIAL — the colored strand must be slightly TRANSLUCENT like real thin extension hair: the "
        f"underlying blonde hair's bright shine and lighter highlight streaks must remain VISIBLE showing "
        f"THROUGH the colored strand (just like a thin pink extension lets the blonde glow through), so "
        f"the strand is brighter and lighter in places, never a single opaque saturated block. "
        f"Do NOT make it a dense opaque neon fill — reduce the saturation a touch and let light pass through. "
