...
def build_clipin_before(clothing_en="", match_identity=True):
    ...
    if match_identity:
        identity = (
            "TASK: take the EXACT woman shown in the FIRST image and simply re-pose her. The output is a "
            "photo of the FIRST image's woman — her exact face, her exact skin tone, her exact natural hair "
            "color, her exact outfit. Keep her 100% identical to the first image in every way EXCEPT her "
            "body pose. "
            "The SECOND image is NOT a person — treat it as a pure POSE DIAGRAM / mannequin showing only the "
            "body posture, turn angle and arm position to copy. The woman in the second image is a "
            "COMPLETELY DIFFERENT, unrelated person who must NOT appear in the result at all: do NOT copy her "
            "face, do NOT copy her skin tone, do NOT copy her hair color or hairstyle, do NOT copy her "
            "clothing. Borrow ONLY the body pose from the second image. "
            "CRITICAL IDENTITY CHECK: the result must look unmistakably like the SAME woman as the FIRST "
            "image and NOTHING like the woman in the second image. If the first image's woman has tan / "
            "medium-brown skin and dark hair, the result MUST keep that exact tan skin and dark hair — never "
            "output a lighter-skinned, fair, or different-faced woman from the second image. "
        )
        clothing_override = ""
    else:
...
def build_clipin_studio(color_name, hex_color):
    ...
    return (
        f"TASK: take the EXACT woman shown in the FIRST image and RE-POSE her body into the NEW pose shown in "
        f"the SECOND image. The most important change is the POSE: her body posture, the way her torso is "
        f"turned, her head tilt and her arm/hand position MUST change to MATCH the pose in the second image. "
        f"Do NOT keep her original standing pose from the first image — she must clearly take on the second "
        f"image's pose (for example, if the second image shows the body turned slightly with one hand lifted "
        f"up touching the hair near the face, then in the result she also turns and lifts her hand up to her "
        f"hair the same way). The output pose must visibly differ from the first image and visibly copy the "
        f"second image's pose. "
