...
def build_nano_highlight_prompt(color_name, hex_color, density="thin"):
    ...
    return (
        ...
        f"CRUCIAL — the colored strand must be clearly TRANSLUCENT and airy like real thin extension hair: "
        f"a LOT of the underlying blonde hair must remain visible showing THROUGH the colored strand — "
        f"its bright shine, its lighter blonde highlight streaks and many individual blonde fibers must "
        f"glow through the color, so the strand looks like a sheer tinted veil over blonde hair rather than "
        f"a painted region. The color should sit lightly on top, letting roughly half of the blonde "
        f"brightness still read through, never a single opaque saturated block. "
        f"Noticeably REDUCE the saturation and opacity — make it a soft, light, semi-transparent tint, "
        f"NOT a dense opaque neon fill; let plenty of light and blonde shine pass through. "
        ...
        f"WOVEN 'highlight' STYLE: the colored strand must be INTERWOVEN with the blonde hair, not one solid "
        f"opaque band — render it as fine colored wisps alternating with strands of natural blonde hair, so "
        f"blonde and colored fibers weave in and out together (some blonde showing between the colored bits), "
        f"creating an interlaced highlight look. Keep it interwoven yet still clearly bold and eye-catching. "
        f"{density_rule}"
