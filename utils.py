...
def build_clipin_before(clothing_en="", match_identity=True):
    ...
    return (
        ...
        f"CHANGE 1 — REMOVE ALL HIGHLIGHTS AND MAKE THE HAIR ONE SOLID UNIFORM NATURAL COLOR: her hair must "
        f"become COMPLETELY ONE single solid natural color, the SAME shade from the roots all the way to the "
        f"tips. Remove every colored highlight strand (pink/blue/green/red) 100%, AND ALSO remove any ombre, "
        f"balayage, dip-dye, silver/ash gradient, lightened ends, two-tone effect or any color variation in "
        f"the hair. There must be NO lighter or different-colored sections anywhere — the ends must be the "
        f"EXACT same natural color as the roots. Pick her natural base color (if her roots are dark brown, "
        f"make the whole hair that dark brown; if naturally blonde, an even natural blonde) and apply it "
        f"evenly to every strand. The before hair looks like plain, untreated, single-color natural hair "
        f"with NO highlights, NO ombre and NO color gradient at all. This is mandatory.\n"
