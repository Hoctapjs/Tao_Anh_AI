...
def build_nano_highlight_prompt(color_name, hex_color, density="thin", with_swatch=True):
    ...
    return (
        ...
        f"COLOR INTENSITY: the colored fibers themselves must be RICH, VIVID and fully saturated, matching "
        f"the bright bold {color_name} of the swatch (hex {hex_color}) — do NOT wash them out, do NOT make "
        f"them pale, milky, pastel or faded. Each individual colored hair is a deep, strong {color_name}. "
        f"The airy, light look must come ONLY from blonde hairs woven BETWEEN the colored ones, NOT from "
        f"desaturating or fading the color. So: vivid saturated colored strands interlaced with blonde "
        f"strands — the green stays punchy and true to the swatch, just broken up by blonde threads. "
        f"Keep natural shine and light-and-shadow on the colored fibers (glossy where light hits, deeper in "
        f"shadow), but the hue stays bold and matches the swatch, not a faint tint. "
