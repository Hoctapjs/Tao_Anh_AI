...
def build_clipin_after_prompt(color_name, hex_color):
    ...
    return (
        ...
        f"Weave in several THIN clip-in colored highlight extension strands matching the color shown in the "
        f"second image (the color swatch): {color_name}, hex {hex_color}. "
        f"The colored strands must START high up near the crown and the hair parting line at the top of the "
        f"head, and run as long continuous straight vertical streaks flowing DOWN ALONG the natural hair "
        f"all the way to the very ends — visible from the top of the head down to the tips, exactly like "
        f"real clip-in hair extensions. "
        f"Spread several SLIM separate colored strands scattered and interleaved among the dark hair on "
        f"BOTH sides framing the face (not just one side), evenly distributed and blended between the dark "
        f"strands. Do NOT bunch them into one solid thick block or patch on a single spot; do NOT start "
        f"them only at the middle of the hair; do NOT make a curved swirl. "
        f"Keep them SUBTLE and SPARSE — thin individual strands, the colored hair must be a small fraction "
        f"of the total hair. The colored strands must be the SAME perfectly STRAIGHT smooth texture and "
        f"length as the surrounding natural hair — keep the hair straight exactly like the before photo, "
        f"do NOT make it wavy or curly. "
